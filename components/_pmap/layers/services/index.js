...
class Services extends BasicMapApi {
  ...
  /**
   * 获取Arcgis WMTS类型服务的图层对象
   * @param {String} id 图层Id名称
   * @param {String} url 服务地址
   * @param {Object} options 解析服务的参数选项
   */
  async getArcgisWMTSLayer(id, url, options = {}) {
    const result = validateServicesOptions(id, url, options);
    if (!result) {
      return null;
    }
    let arcgisWmts = new ArcgisWMTS(this.iMapApi);
    const layer = arcgisWmts.getLayer(result.id, result.name, result.url, result.options);
    arcgisWmts = null;
    return layer;
  }
  ...
  /**
   * 获取Arcgis WMS类型服务的图层对象
   * @param {String} id 图层Id名称
   * @param {String} url 服务地址
   * @param {Object} options 解析服务的参数选项
   */
  async getArcgisWMSLayer(id, url, options = {}) {
    const result = validateServicesOptions(id, url, options);
    if (!result) {
      return null;
    }
    let arcgisWms = new ArcgisWMS(this.iMapApi);
    const layer = arcgisWms.getLayer(result.id, result.name, result.url, result.options);
    arcgisWms = null;
    return layer;
  }
   ...
   * @param {Object} options 解析服务的参数选项
   */
  async getArcgisExportLayer(id, url, options = {}) {
    const result = validateServicesOptions(id, url, options);
    if (!result) {
      return null;
    }
    let arcgisExport = new ArcgisExport(this.iMapApi);
    const layer = arcgisExport.getLayer(result.id, result.name, result.url, result.options);
    arcgisExport = null;
    return layer;
  }
