...
除通过配置文件形式渲染`WMS`服务图层外，我们还可以通过`iMapApi.Services`二级对象下的接口进行服务图层叠加。
...
- #### iMapApi.Services.addServicesLayer(type, id, url, options?, beforeId?, layerGroupId?)
	...
	**options** - `WMS`服务参数选项
	...
	| version | 选填项，WMS服务版本号 | `1.1.1` \| `1.3.0` | '1.0.0' |
	| layerName | 选填项，WMS服务图层名标识，多个以`,`逗号分割 | string | null |
	| styleName | 选填项，WMS服务图层样式名 | string | null |
	| format | 选填项，WMS服务的图片格式 | `image/gif` \| `image/png` \| `image/jpeg` | 'image/png' |
