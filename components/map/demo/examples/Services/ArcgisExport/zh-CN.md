...
除通过配置文件形式渲染`ArcgisExport`服务图层外，我们还可以通过`iMapApi.Services`二级对象下的接口进行服务图层叠加。
...
- #### iMapApi.Services.addServicesLayer(type, id, url, options?, beforeId?, layerGroupId?)
	...
	**options** - `ArcgisExport`服务参数选项
	...
	| layerName | 选填项，Arcgis Export服务图层索引，多个以`,`逗号分割 | string | null |
	| srs | 选填项，输入空间坐标系统 | string | null |
	| outSRS | 选填项，输出空间坐标系统 | string | null |
	| layerDefs | 选填项，图层过滤条件 | string | null |
	| format | 选填项，Arcgis Export服务的图片格式 | `image/gif` \| `image/png` \| `image/jpeg` | 'image/png' |
